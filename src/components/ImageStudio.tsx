...
import { ArrowDownToLine, ArrowRight, AudioLines, Check, CircleHelp, Download, Expand, FileAudio, ImagePlus, Layers3, LoaderCircle, LockKeyhole, Monitor, Pause, PenLine, Play, RotateCcw, ScanLine, ShieldCheck, SlidersHorizontal, Terminal, Upload, Volume2, VolumeX, X } from 'lucide-react';
import { artworkMarkup } from '../lib/artwork';
import { downloadBlob } from '../lib/export';
import { formatTime } from '../lib/project';
import { defaultSketchSettings, downloadSketchPng, extractContours, loadMarker, readAudio, readImage, recordSketch, SketchPainter, sketchVideoFormat } from '../lib/imageSketch';
import type { AudioInput, ImageInput, SketchDrawing, SketchSettings } from '../lib/imageSketch';
...
function SketchCanvas({ drawing, settings, progress, marker }: { drawing: SketchDrawing; settings: SketchSettings; progress: number; marker?: HTMLCanvasElement }) {
  ...
  return <canvas className="sketch-canvas" ref={canvas} width={1440} height={810} role="img" aria-label="Progressive contour sketch of your uploaded image"/>;
}
...
export default function ImageStudio({ active, helpRequest, notify, onScriptStudio }: Props) {
  ...
  const [drawing, setDrawing] = useState<SketchDrawing>();
  ...
  const imageUpload = useRef<HTMLInputElement>(null), audioUpload = useRef<HTMLInputElement>(null);
  ...
  const play = useCallback(async () => {
    if (!audio || !drawing || !audioElement.current) { notify('Add an image and audio, then create your sketch to preview it.'); return; }
  ...
  }, [audio, drawing, notify, pause, playing, time]);
  ...
  const invalidateDrawing = () => { processingAbort.current?.abort(); setProcessing(false); setDrawing(undefined); setExportResult(undefined); resetPlay(); };
  ...
  const pickImage = async (file?: File) => {
    ...
    try {
      ...
      if(ticket !== imageTicket.current) { URL.revokeObjectURL(loaded.url);return; }
      setImage(loaded);setView('original');
  ...
  const drop = (event: DragEvent, type: 'image' | 'audio') => {
    ...
    if(event.dataTransfer.files.length!==1){setError('Drop one file at a time.');return;}
    if(type==='image')void pickImage(event.dataTransfer.files[0]);else void pickAudio(event.dataTransfer.files[0]);
  };
  const createSketch = async () => {
    if(!image){imageUpload.current?.click();return;}
    ...
    try {
      const result=await extractContours(image.pixels,settings,controller.signal,setProcessingProgress);
      if(controller.signal.aborted)return;
      setDrawing(result);setView('sketch');notify(`${result.paths.length.toLocaleString()} strokes, traced from your image. ${audio?'Ready to play with your audio.':'Add audio to bring your sketch to life.'}`);
  ...
  const sample = async () => {
    try {
      ...
      try{
        ...
        if(blob)await pickImage(new File([blob],'rainwater-diagram.png',{type:'image/png'}));
      }finally{URL.revokeObjectURL(url);}
    }catch{setError('The sample could not be loaded. Try uploading your own image.');}
  };

  const openExport = () => {
    if(!image||!audio||!drawing){notify('Add your image and audio, then create a sketch before exporting.');return;}
  ...
  const startExport = async () => {
    ...
    try {
      if(exportMode==='python') {await downloadSketchKit(image?.file,audio?.file,settings,controller.signal);setKitDownloaded(true);notify('Your image, audio, and Python engine are in the render kit.');}
      else {
        if(!drawing||!audio)throw new Error('Add an image and audio before recording.');
        const result=await recordSketch(drawing,audio,settings,marker,setExportProgress,controller.signal);
        if(controller.signal.aborted)return;
        setExportResult(result);downloadBlob(result,`whiteboard-sketch.${recordingFormat?.extension||'webm'}`);notify('Your sketch video includes your uploaded audio. Check your downloads.');
      }
  ...
  const renderPreview = () => {
    ...
    if(view==='original'&&image)return <div className="original-image-plane" style={{background:settings.paper}}><img src={image.url} alt="Original uploaded image"/></div>;
    if(drawing)return <SketchCanvas drawing={drawing} settings={settings} progress={fraction} marker={marker}/>;
    if(image)return <div className="original-image-plane" style={{background:settings.paper}}><img src={image.url} alt="Uploaded image, ready for contour extraction"/></div>;
    return <div className="sketch-empty-plane"><div className="empty-line-drawing" aria-hidden="true"><svg viewBox="0 0 200 120" fill="none"><path d="M52 90V36q0-6 6-6h84q6 0 6 6v54q0 6-6 6H58q-6 0-6-6Z" stroke="currentColor" strokeWidth="1.6" strokeDasharray="5 5"/><path d="m57 88 28-26 21 19 17-15 20 21M115 48a7 7 0 1 0 14 0a7 7 0 1 0-14 0" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/><path d="m153 59 23-20 6 7-23 20-10 2 4-9ZM172 43l6 7" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/><path d="M30 49h7M33 45v8M164 91h8M168 87v8" stroke="currentColor" strokeLinecap="round"/></svg></div><h3>Your picture has a story to tell.</h3><p>Bring an image. Add your voice. We will draw the rest.</p><button className="text-button" onClick={()=>void sample()}>Try a sample image <ArrowRight size={14}/></button></div>;
  };

  return <motion.div className="image-studio" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{duration:.35}}>
    <input className="sr-only" ref={imageUpload} type="file" accept=".png,.jpg,.jpeg,.webp,.bmp" aria-label="Upload an image" onChange={e=>void pickImage(e.target.files?.[0])}/>
    ...
    <div className="workflow-tabs image-workflow" aria-label="Image to video workflow"><div className="workflow-step"><button className={!image?'selected':''} onClick={()=>imageUpload.current?.click()}><span className="step-number">{image?<Check size={12}/>:'01'}</span>Your image</button><span className="step-connector"/></div><div className="workflow-step"><button className={image&&!audio?'selected':''} onClick={()=>audioUpload.current?.click()}><span className="step-number">{audio?<Check size={12}/>:'02'}</span>Your audio</button><span className="step-connector"/></div><div className="workflow-step"><button className={image&&audio?'selected':''} onClick={()=>controls.current?.scrollIntoView({behavior:'smooth',block:'center'})}><span className="step-number">03</span>Make it a sketch</button></div><span className="workflow-aside"><ShieldCheck size={14}/>Your files never leave your device.</span></div>
    <div className="studio-grid image-studio-grid">
      <section className="editor-panel image-input-panel" aria-label="Image and audio inputs">
        ...
        <div className="upload-field-heading"><label htmlFor="choose-sketch-image">Your image</label><span>PNG, JPG, WebP</span></div>
        <div className={`media-drop ${image?'has-file':''} ${dragOver==='image'?'dragging':''}`} onDragOver={e=>{e.preventDefault();setDragOver('image');}} onDragLeave={()=>setDragOver(null)} onDrop={e=>drop(e,'image')}>
          {image&&!loadingImage?<><img className="uploaded-image-thumb" src={image.url} alt=""/><div className="uploaded-file-info"><strong title={image.file.name}>{image.file.name}</strong><span>{image.originalWidth.toLocaleString()} x {image.originalHeight.toLocaleString()} px</span><button className="text-button" onClick={()=>imageUpload.current?.click()}>Replace image</button></div><button className="icon-button remove-file" title="Remove image" aria-label="Remove image" onClick={()=>{imageTicket.current++;setImage(undefined);invalidateDrawing();setError('');}}><X size={14}/></button></>:<button id="choose-sketch-image" className="drop-target" disabled={loadingImage} onClick={()=>imageUpload.current?.click()}>{loadingImage?<LoaderCircle className="spin" size={24}/>:<ImagePlus size={25} strokeWidth={1.4}/>}<strong>{loadingImage?'Opening your picture...':'Drop an image, or browse'}</strong><span>Diagrams, photographs, or your own artwork.</span></button>}
        </div>
        ...
        <div className="editor-bottom"><button className="primary-button generate-button" disabled={processing||loadingImage||loadingAudio} onClick={()=>void createSketch()}>{processing?<LoaderCircle className="spin" size={17}/>:<ScanLine size={17}/>}<span>{processing?'Tracing your picture...':drawing?'Recreate sketch':'Create sketch'}</span><ArrowRight size={17}/></button><p className="privacy-caption"><LockKeyhole size={11}/>No AI. No API keys. Just your creativity.</p></div>
      </section>
      <section className="preview-panel image-preview-panel" aria-label="Image sketch preview"><div className="preview-toolbar"><h2><span className="live-dot"/>Your picture, taking shape</h2><div><span className="aspect-label"><Monitor size={14}/>16:9</span><span className="preview-toolbar-divider"/><button className="icon-button" aria-label="Expand sketch preview" title="Expand preview" onClick={()=>setDialog('full')}><Expand size={16}/></button></div></div><div className="image-preview-stage">{renderPreview()}</div><div className="image-preview-switches"><div className="preview-mode-switch"><button disabled={!drawing} className={view==='sketch'?'active':''} onClick={()=>setView('sketch')}><PenLine size={12}/>Sketch</button><button disabled={!image} className={view==='original'?'active':''} onClick={()=>{pause();setView('original');}}><ImagePlus size={12}/>Original</button></div><div className="paper-mini-options" aria-label="Canvas background">{[{color:'#fcfbf5',name:'Warm paper'},{color:'#ffffff',name:'Pure white'},{color:'#f0f3ed',name:'Soft sage'}].map(p=><button key={p.color} title={p.name} aria-label={`Use ${p.name.toLowerCase()} canvas`} aria-pressed={settings.paper===p.color} style={{background:p.color}} className={settings.paper===p.color?'selected':''} onClick={()=>updateSettings({paper:p.color})}>{settings.paper===p.color&&<Check size={9}/>}</button>)}</div></div><div className="playback-controls"><button className="play-button" aria-label={playing?'Pause sketch preview':'Play sketch with audio'} title={audio&&drawing?'Play your picture with audio':'Add image and audio to preview'} disabled={!audio||!drawing} onClick={()=>void play()}>{playing?<Pause size={14} fill="currentColor"/>:<Play size={14} fill="currentColor"/>}</button><button className="icon-button restart-button" aria-label="Restart sketch" title="Restart sketch" onClick={()=>{pause();if(audioElement.current)audioElement.current.currentTime=0;setTime(0);setStarted(true);setView('sketch');}}><RotateCcw size={14}/></button><span className="playback-time">{formatTime(time)} <span>/ {formatTime(duration)}</span></span><input className="timeline-scrubber" aria-label="Sketch playback position" type="range" min={0} max={duration||1} value={time} step={.01} disabled={!audio||!drawing} style={{'--progress':`${duration?time/duration*100:0}%`} as CSSProperties} onChange={e=>{pause();const value=Number(e.target.value);setStarted(true);setView('sketch');setTime(value);if(audioElement.current)audioElement.current.currentTime=value;}}/><button className="icon-button" aria-label={muted?'Unmute supplied audio':'Mute supplied audio'} title={muted?'Unmute audio':'Mute audio'} onClick={()=>setMuted(!muted)}>{muted?<VolumeX size={16}/>:<Volume2 size={16}/>}</button></div><div className="preview-caption"><span><Layers3 size={12}/>{drawing?`${drawing.paths.length.toLocaleString()} real image strokes`:'Your canvas is ready'}</span><span>A picture, one little line at a time.</span></div></section>
    </div>
    <section className="audio-timeline-section" aria-label="Audio-synchronized drawing timeline"><div className="storyboard-heading"><div><h2>Your story, in sync</h2><span className="scene-count">{audio?`${duration.toFixed(2)} seconds`:'Waiting for your audio'}</span></div><button className="text-button" disabled={!drawing} onClick={()=>{if(drawing)downloadSketchPng(drawing,settings);}}><Download size={13}/>Save sketch</button></div><div className={`sketch-audio-timeline ${audio?'has-audio':''}`}><div className="timeline-track-icon"><AudioLines size={21} strokeWidth={1.3}/><span>Audio</span></div>{audio?<button className="waveform-track" aria-label="Seek within your supplied audio" disabled={!drawing} onClick={e=>{const bounds=e.currentTarget.getBoundingClientRect();const value=Math.max(0,Math.min(duration,(e.clientX-bounds.left)/bounds.width*duration));pause();setTime(value);setStarted(true);if(audioElement.current)audioElement.current.currentTime=value;}}><div className="waveform-bars">{audio.peaks.map((peak,i)=><span key={i} style={{height:`${Math.max(5,peak*78)}%`,background: i/100<time/duration?'#6d8955':undefined}}/>)}</div><div className="audio-playhead" style={{left:`${time/duration*100}%`}}/><span className="waveform-zero">0:00</span><span className="waveform-end">{formatTime(duration)}</span></button>:<button className="timeline-add-audio" onClick={()=>audioUpload.current?.click()}><Upload size={15}/><span>Your audio sets the pace. Add a recording to get started.</span></button>}<div className="timeline-frame-info"><strong>{audio?totalFrames.toLocaleString():'--'}</strong><span>frames at {settings.fps} fps</span></div></div><p className="timeline-footnote">{audio?'Drawing speed adapts to your audio. No voice generation, stretching, or trimmed endings.':'Your original audio will be included in both browser and Python video exports.'}</p></section>
